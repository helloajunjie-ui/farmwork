...
/**
 * 获取指定等级（含）的累计房产投入
 * 用于净值计算：NetWorth = gold + landValue + inventoryValue + totalHousingCost
 */
export function getAccumulatedHousingCost(tier: number): number {
  return HOUSING_TIERS[Math.max(0, Math.min(tier - 1, 19))].totalCost
}
